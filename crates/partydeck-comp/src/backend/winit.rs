...
pub fn init(
    ...
    if let Some(app_id) = steam::app_id() {
    ...
    let dmabuf_state = match render_node {
        Ok(Some(node)) => {
            ...
            let global = dmabuf_state
                .create_global_with_default_feedback::<CompState>(display_handle, &feedback);
        ...
        .insert_source(winit, move |event, _, data| {
            match event {
                WinitEvent::Resized { size, .. } => {
                    slots::handle_output_resize(&mut data.state, size)
                }
