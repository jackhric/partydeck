...
use partydeck_comp_proto::state::BorderStyle;
...
fn load_layout(args: &Args) -> Result<Layout, Box<dyn std::error::Error>> {
    let layout = match &args.layout {
        Some(path) => Layout::read(path).map_err(|e| format!("layout {}: {e}", path.display()))?,
        None => presets::quadrants(args.players),
    };
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    insert_render_timer(&mut event_loop)?;
    let control_path = ipc::init(&mut event_loop, &args.socket_prefix)?;
    ...
    wayland::sockets::remove_socket_files(&data.state.socket_names);
    ipc::remove_socket_files(&control_path);
    Ok(())
}
...
fn insert_render_timer(
    ...
    event_loop
        ...
        .insert_source(Timer::immediate(), move |_, _, data| {
            data.state.telemetry.report();
            let now = Instant::now();
            if data.state.host_ready {
                data.state.host_ready = false;
                render::redraw(&mut data.state, &mut data.display_handle, now);
            } else if now.duration_since(data.state.last_composite_at)
                > render::feedback::STALL_TIMEOUT
            {
                render::feedback::stall_tick(&mut data.state, &mut data.display_handle);
            }
            let frame_interval = render::frame_interval(&data.state);
