...
import BrandMark, { LOGO_DUR, LOGO_HOLD, STAGGER } from "./BrandMark";
...
const CARD_DELAY = LOGO_HOLD + LOGO_DUR; // card enters as the brand mark settles
const CARD_SLIDE = 32; // px the icon/name travel leftward into place

// Icon + name share one motion element so they slide in as a single unit.
...
export default function SlotCover({
  ...
  // Latch: a later live:false (compositor hiccup) must never re-show the cover.
  ...
  return (
    <AnimatePresence>
      {!lifted && (
        ...
        >
          <BrandMark avatar={avatar} index={index} />
          {label && (
            <motion.div
            ...
            >
              ...
              <span className="pd-text text-neutral-400 text-[min(4vw,1.5rem)] font-bold">
                {label}
              </span>
            </motion.div>
          )}
