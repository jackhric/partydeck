...
// PartyDeck brand mark (the "P" glyph) shown on the slot cover while a game
// loads. Not to be confused with slot.logo, the game's own logo.

// Fade sequence (seconds): black hold, mark fades in, gradient blooms over it.
...
const GRADIENT_DELAY = LOGO_HOLD + LOGO_DUR;
...
// Whole-mark viewBox, and the translate that maps the raw path coords into it.
const VB_W = 48.947636;
const VB_H = 39.299686;
const PATH_OFFSET = "translate(-24.70456,-86.867724)";

// Center of the P's counter (the hole) as a % of the box; the bloom rotates about it.
const HOLE_X_PCT = 46;
const HOLE_Y_PCT = 20;

const BLOOM_DUR = 6000; // ms per full rotation

// A conic gradient is angular, so rotating it loops with no seam; the palette
// is closed (last stop == first) so the 360 degree join is smooth too.
...
export default function BrandMark({
  ...
  // Timer-based phase (not frame-count) so the bloom speed is independent of paint rate.
  ...
  return (
    <motion.svg
      ...
      transition={{
        // Per-value transitions replace the parent config, so the delay must live in each.
        opacity: { duration: LOGO_DUR, delay, ease: "easeOut" },
        scale: { type: "spring", stiffness: 90, damping: 20, delay },
      }}
    >
      <defs>
        {/* Clip in SVG user space so the gradient registers exactly with the white path. */}
        <clipPath id={`${gradId}-clip`} clipPathUnits="userSpaceOnUse">
          <path transform={PATH_OFFSET} d={TOP_LEFT} />
        </clipPath>
      </defs>
      {/* White base. The P carries a thin background-colored stroke over its fill,
          eroding its edge uniformly inward so no white fringe peeks past the gradient's clip. */}
      <g transform={PATH_OFFSET}>
      ...
      {/* SVG fills cannot take a CSS gradient, so the bloom is an HTML div clipped to the glyph. */}
      <foreignObject
        x={0}
        y={0}
        width={VB_W}
